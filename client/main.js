	...
	fetch('/list')
		...
		.then(files => {
			...
			if (files.length) {
				...
				emptyListItem.textContent = 'n0 f1l3s f0und.';
				// add the list item to the list
...
function setStatic(id, visible) {
    var img = document.getElementById("static");
    img.style.visibility = (visible ? 'visible' : 'hidden');
}
function setRec(id, hidden) {
